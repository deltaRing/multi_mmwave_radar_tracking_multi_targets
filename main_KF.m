...
for xx = 1:fnumber - 1
    ...
    if isempty(new_data)
        ...
        LocX = {}; LocY = {};
        for iiii = 1:length(singleMeasures)
            for jjjj = 1:length(singleMeasures{iiii})
                locx = []; locy = [];
                for kkkk = 1:length(singleMeasures{iiii}{jjjj})
                    if singleMeasures{iiii}{jjjj}{kkkk}(1) == xx * dt
                        locx = [locx singleMeasures{iiii}{jjjj}{kkkk}(2)]; 
                        locy = [locy singleMeasures{iiii}{jjjj}{kkkk}(3)];
                    end
                end
                LocX{end + 1} = locx; LocY{end + 1} = locy;
            end
        end
        ...
        if isempty(Tracks)
            for tt = 1:length(LocX)
                ...
                if isnan(locX) || isnan(locY)
                    ...
                    Tracks{end + 1} = trackInit([locX locY], [0 0], dt, xx, ID);
                end
                ID = ID + 1;
            end
        else
